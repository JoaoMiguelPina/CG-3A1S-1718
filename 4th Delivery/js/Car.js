...
class Car extends GameObject{
	...
	constructor (x,y,z, materialPilot, materialTop, materialWheel, speed, accel){
     ...
     super(x, y, z, speed, accel, 26);
		...
		this.addCarWheel(materialWheel, 15, 5, 0);
		this.addCarLight(5, 9, 10);
		this.addCarLight(5, -9, 10);

	    this.butterCollision = false;
		this.lives = 5;
		this.initLives();
		sceneHUD.add(carLives);
  	...
  	initLives(){
		...
		var x = -700, y = 325;
		for (var i = 0; i < 5; i++){
			this.createCarLive(x,y,0);
			y -= 80;
		}
	}

	createCarLive(x,y,z){
		var geometry = new THREE.PlaneGeometry(70, 50, 0);

		var obj = new THREE.Object3D();

		var texture = new THREE.TextureLoader().load("textures/carrotex.png");

	    var material = new THREE.MeshBasicMaterial( { map: texture, transparent: true} );

	    var vida = new THREE.Mesh(geometry, material);

			obj.add(vida);

	    vida.position.set(x, y, z);
	    carLives.add(obj);
	...
	updateObjectMovement(delta){
		...
		if (this.position.x > 650 || this.position.x < -625 || this.position.y > 375 || this.position.y < -375){
			if(this.lives > 0){
				this.lives--;
				carLives.children[this.lives].visible = false;
	...
	collision(obj){
			...
			if(obj instanceof Orange) { /* Orange Collision */
				if(obj.visible == true){
					...
					this.speed = 0;
					this.rotation.set(0,0,0);
					console.log('choquei numa laranja');
					if(this.lives > 0){
						this.lives--;
						carLives.children[this.lives].visible = false;
	...
	addCarLight(x, y, z) {
		var spotLight1 = new THREE.SpotLight( 0xffffff,3, 300, Math.PI/5, 1, 1);
		spotLight1.rotateY(Math.PI /4);
		spotLight1.angle = Math.PI / 5;
		spotLight1.target.position.set(100,0,0);
		this.add(spotLight1);
		spotLight1.position.set(x, y, z);
		this.add(spotLight1.target);
	}
